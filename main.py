...
الصور المتعددة بتتعالج بالتوازي (concurrently) عشان نداءات PlantNet/Roboflow
الشبكية متبقاش بتستنى بعضها، وبعدين الرد بيبقى متوسط نتايج كل الصور.
...
async def analyze(
    ...
    # ================= UPLOAD PATH (بالتوازي) =================
    if source == "upload":
        results = await asyncio.gather(*[
            asyncio.to_thread(process_upload_image, pil_img)
            for pil_img, _ in pil_images
        ])
        return {"source": "upload", **aggregate_upload_results(list(results))}

    # ================= CAMERA PATH (بالتوازي) =================
    results = await asyncio.gather(*[
        asyncio.to_thread(process_camera_image, pil_img, raw)
        for pil_img, raw in pil_images
    ])
    return {"source": "camera", **aggregate_camera_results(list(results))}
